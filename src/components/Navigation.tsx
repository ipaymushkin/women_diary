    ...
    <header className="sticky top-0 z-40 bg-white border-b border-gray-100 shadow-xs mr-vw">
      <div className="max-w-6xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <div className="w-10 h-10 rounded-full bg-brand flex items-center justify-center text-white font-serif font-bold text-lg shadow-sm">
            Я
          </div>
          <div>
            <h1 className="text-md md:text-lg font-bold text-emerald-950 tracking-tight leading-none">
              Дневник Трезвости
            </h1>
            <span className="text-xs text-brand-dark font-medium font-sans">
        ...
        <div className="flex items-center space-x-2">
          <div className="bg-brand-light border border-brand/20 px-3 py-1.5 rounded-2xl flex items-center space-x-2 text-xs md:text-sm text-brand-dark font-medium shadow-2xs">
            <Calendar size={14} className="text-brand" />
            <span>{formatRussianDate(selectedDate)}</span>
            {isToday ? (
              <span className="bg-brand text-white text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded-md">
                Сегодня
              </span>
            ) : null}
          </div>
          ...
          {!isToday && (
            <button
              ...
              className="p-1.5 md:p-2 rounded-2xl border border-gray-100 hover:border-brand-medium hover:bg-brand-light bg-white text-brand-dark hover:text-brand-dark transition-all cursor-pointer shadow-2xs"
